...
- This exact combination — rootless Podman, the `krun` runtime, bind-mounted staging-directory storage, and `passt` networking (see 2.3) — doesn't have a well-trodden public reference to follow end to end, so it needs real hands-on validation before launch, not just a configuration change.

### 2.2 The workspace: how files move in, out, and back

**Storage.** Each session gets its own disposable, git-seeded staging directory, bind-mounted into the guest at `/workspace` — never a live share or direct link to your real folder. This needs no privileged host-side mount step (no loop device, no root-owned mountpoint), which keeps it consistent with the rootless launch model in 2.1. The agent works against that staging directory, and only that staging directory; your real project folder is never itself the bind-mount source.
...
| File sharing | A disposable, bind-mounted staging directory per session | Avoids ever bind-mounting or otherwise live-sharing your real project folder — removing that as a way to break out. Trade-off: the disposable staging copy itself is now continuously bind-mounted for the life of the session, handled by the sync mechanism above rather than by sealing it at boot. |
...
- **This exact technology combination is newer and less proven** — rootless Podman + `krun` + bind-mounted staging-directory storage + `passt` networking doesn't have a well-trodden public reference to follow end to end, and needs real validation on real hardware before v1 ships. DNS containment under `passt` specifically needs to be verified so it doesn't leak a path around the network restriction in 2.3.
