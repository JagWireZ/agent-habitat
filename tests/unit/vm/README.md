...
run` argv, and teardown actually deletes the session's staging directory
and force-removes its container, idempotently, leaving no residual
writable artifact reachable from a later session.
...
containment story (the only bind mount ever built into the argv is the
expected session staging directory, never `project_root` or any other
host path, and no host-privilege-widening flag is ever built in) lives in
