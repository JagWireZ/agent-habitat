...
- `blocklist_disk_image.rs` (Phase 2): seeds a project with a variant of
  ...
  a unique secret value, runs the real staging-build pipeline, then
  confirms none of those files or secret values appear anywhere in the
  staging directory -- the same directory that is bind-mounted into the
  guest, so checking it directly *is* checking what the guest sees.
  Unlike Phase 3/5's KVM-dependent adversarial tests, this one needs no
  real hardware -- `git` is ordinary tooling present in this dev
...
- `containment_escape.rs` (Phase 3): the mocked-launch-seam half of the
  ...
  never widens host privilege (`--privileged`, `--cap-add`, `--pid=host`,
  `--network=host`, and similar), defaults the guest's network to
  explicitly `none` rather than an unset default, and that the *only*
  bind mount (`-v`, `--mount type=bind`) present is the expected session
  staging directory -- `project_root` (or any other arbitrary host path)
  never appears as a bind-mount source. This is *not* the exit gate's real
  escape-attempt test -- reaching host files/processes/network from inside
  an actually-booted guest needs real KVM, which neither this dev
  container nor this project's CI has, so that attempt is
